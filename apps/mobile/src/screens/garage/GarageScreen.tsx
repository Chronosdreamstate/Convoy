...
import {
  ...
  ScrollView, ActivityIndicator, TextInput, Modal, Alert, RefreshControl,
} from 'react-native';
...
const TYPE_EMOJI: Record<string, string> = {
  Car: '🚗', Truck: '🚚', Motorcycle: '🏍️', SUV: '🚙', Classic: '🏎️',
};
const VEHICLE_TYPES = ['Car', 'Truck', 'Motorcycle', 'SUV', 'Classic'] as const;
...
interface VehicleForm { name: string; type: string; year: string; }
const EMPTY_FORM: VehicleForm = { name: '', type: 'Car', year: '' };
...
function vehicleSubtitle(v: Vehicle): string {
  return [v.year ? String(v.year) : null, v.type ?? v.color].filter(Boolean).join(' · ');
}
...
export default function GarageScreen() {
  ...
  const loadVehicles = async (silent = false) => {
    ...
    setError(null);
    try {
      const res = await apiClient.get<{ vehicles: Vehicle[] }>('/api/v1/vehicles');
  ...
  const openEditModal = (v: Vehicle) => {
    setEditingId(v.id);
    setForm({ name: v.name ?? vehicleDisplayName(v), type: v.type ?? 'Car', year: v.year != null ? String(v.year) : '' });
    setFormError(null);
  ...
  const handleSave = async () => {
    setFormError(null);
    if (!form.name.trim()) { setFormError('Please enter a vehicle name.'); return; }
    ...
    const payload = { name: form.name.trim(), type: form.type, ...(yearNum != null ? { year: yearNum } : {}) };
    setIsSaving(true);
    try {
      if (editingId) {
        const res = await apiClient.patch<Vehicle>(`/api/v1/vehicles/${editingId}`, payload);
        setVehicles((prev) => prev.map((v) => (v.id === editingId ? res.data : v)));
      } else {
  ...
  const openActionSheet = (v: Vehicle) => {
    Alert.alert(vehicleDisplayName(v), undefined, [
      { text: '⭐  Set as Primary', onPress: () => void handleSetPrimary(v) },
          ...
          vehicles.map((v) => {
            ...
            return (
              <TouchableOpacity
                ...
                accessibilityLabel={`${vehicleDisplayName(v)}${primary ? ', primary' : ', tap to set primary'}`}
              >
                ...
                <View style={styles.vehicleInfo}>
                  ...
                  <View style={styles.metaRow}>
                    {!primary && (isActivating === v.id
                      ...
                      : <Text style={styles.tapToActivate}>Tap to set primary</Text>
                    )}
...
const styles = StyleSheet.create({
  ...
  vehicleInfo: { flex: 1 },
  vehicleName: { fontSize: 18, fontWeight: '700', color: '#FFFFFF' },
  ...
  modalSheet: { backgroundColor: '#1C1C1C', borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingHorizontal: 24, paddingTop: 12, paddingBottom: 48, maxHeight: '85%' },
  ...
  typePill: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, backgroundColor: '#242424', borderWidth: 1, borderColor: '#2A2A2A' },
